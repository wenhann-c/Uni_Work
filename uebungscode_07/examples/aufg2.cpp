...
int main() {
  ...
  g.print_edges();
  // Capturing input as the chosen number(node) to carry out DFS
  int target = input();

  // Nutzen Sie die {fmt}-Library um das Array auszugeben
  fmt::print("{0}{1}{2}","The following are Depth First Traversal starting from node ", fmt::join(g.depth_search(target), "~>"), '\n');
}
